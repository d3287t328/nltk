...
def _pretty_lu(lu):
    ...
    if "lexemes" in lukeys:
        outstr += f'\n[lexemes] {" ".join(f"{lex.name}/{lex.POS}" for lex in lu.lexemes)}\n'
...
def _pretty_annotation(sent, aset_level=False):
    ...
    if not aset_level:
        ...
        outstr += f"\n[POS] {len(sent.POS)} tags\n"
        outstr += f"\n[POS_tagset] {sent.POS_tagset}\n"
    outstr += f'\n[GF] {len(sent.GF)} relation{"s" if len(sent.GF) != 1 else ""}\n'
    outstr += f'\n[PT] {len(sent.PT)} phrase{"s" if len(sent.PT) != 1 else ""}\n'
    """
    Special Layers
    --------------
    ...
    for lyr in ("NER", "WSL", "Other", "Sent"):
        if lyr in sent and sent[lyr]:
            outstr += f'\n[{lyr}] {len(sent[lyr])} entr{"ies" if len(sent[lyr]) != 1 else "y"}\n'
    outstr += "\n[text] + [Target] + [FE]"
...
class AttrDict(dict):
    ...
    def __getitem__(self, name):
        ...
        return v._data() if isinstance(v, Future) else v
    ...
    def exemplars(self, luNamePattern=None, frame=None, fe=None, fe2=None):
        ...
        if frame is not None or luNamePattern is not None:
            if frame is None or isinstance(frame, str):
                ...
                frames = [self.frame(frame)] if isinstance(frame, int) else [frame]
                if luNamePattern is not None:
